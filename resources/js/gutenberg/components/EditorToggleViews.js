...
export default function ToggleViewsDropdown() {
    ...
    const currentViewTitle = currentTemplate ? currentTemplate.title : __( 'View', 'directorist-gutenberg' );
        ...
        <Dropdown
            ...
            renderToggle={ ( { isOpen, onToggle } ) => (
                ...
                >
                    ...
                    <span>{ sprintf( __( 'Edit %s', 'directorist-gutenberg' ), currentViewTitle ) }</span>
                ...
                <div>
                    {templateLinks.map( ( template ) => (
                        <a
                            key={ template.id }
                            href={ template.url }
                            className={clsx('directorist-gutenberg-toggle-views-dropdown-item', {
                                'directorist-gutenberg-toggle-views-dropdown-item-current': template.is_current,
                            })}
                        >
                            <span>{template.title}</span>
                            { template.is_current && <ReactSVG src={ CheckSolid } /> }
                        </a>
                    ) )}
                </div>
            ) }
