...
function getIconUrl( icon ) {
	const iconPath = `${
		icon.set === 'fa' ? 'font-awesome' : 'line-awesome'
	}/${ icon.name }.svg`;
...
export default function IconPicker( {
	...
	const openModal = () => {
		...
		if ( currentIcon ) {
			const iconSet = currentIcon.includes( 'font-awesome' )
				? 'fa'
				: currentIcon.includes( 'line-awesome' )
				? 'la'
				: 'fa'; // default to 'fa'
	...
	const filteredIcons = useMemo( () => {
		let icons =
			selectedIconSet === 'fa' ? fontAwesomeIcons : lineAwesomeIcons;
			...
			<div className="directorist-gutenberg-icon-picker-preview">
				{ selectedIconData ? (
					...
					>
						...
						<span className="directorist-gutenberg-icon-picker-change">
