...
jQuery( document ).ready( function ( $ ) {
				...
				inputs.forEach( function ( elm ) {
					...
					google.maps.event.addListener(
						...
						function () {
							...
							var searchField = elm.closest(
								'.directorist-search-field'
							);
							if ( searchField ) {
								var latInput = searchField.querySelector(
									'#' + field.lat_id
								);
								var lngInput = searchField.querySelector(
									'#' + field.lng_id
								);
								if ( latInput )
									latInput.value =
							...
							var locationSearch = $( elm ).closest(
	...
	function initOpenStreetMapSearch() {
		...
		input_fields.forEach( function ( field ) {
			...
			$( field.input_elm ).on(
				...
				debounce( function ( event ) {
					...
					var $locationField = $( this ).closest(
						'.directorist-search-field'
					);
					var result_container = field.getResultContainer(
						this,
						field
					);
					var search = $( this ).val();
		...
		var syncLatLngData = function ( context, event, args ) {
			...
			var locationSearch = $locationInput.closest(
				'.directorist-search-location'
			);
			if ( locationSearch.length ) {
				locationSearch.trigger( 'change' );
