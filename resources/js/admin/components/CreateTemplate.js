...
import { animateProgress, createDelay } from '@directorist-gutenberg/utils/utils';
...
export default function CreateTemplate({ createType, onClose }) {
    ...
    const handleCreateTemplate = async () => {
        ...
        try {
            ...
            const progressPromise = animateProgress(setLoadingProgress, 0, 50, PROGRESS_DURATION);
            const [ response ] = await Promise.all([requestPromise, progressPromise]);
            console.log('Template creation success:', response);
            setLoadingProgress(50);

            // Delay 50% to 80%
            const delay1Promise = createDelay(DELAY_DURATION);
            const progress1Promise = animateProgress(setLoadingProgress, 50, 80, PROGRESS_DURATION);
            await Promise.all([delay1Promise, progress1Promise]);

            // Delay 80% to 100%
            const delay2Promise = createDelay(DELAY_DURATION);
            const progress2Promise = animateProgress(setLoadingProgress, 80, 100, PROGRESS_DURATION);
    ...
    const renderDirectoryTypeSelector = () => {
        if (createType !== 'all') {
            return <span>{__('Select a template type', 'directorist-gutenberg')}</span>;
        }

        return (
            <div className="directorist-gutenberg-choose-directory-type">
                <h2>{__('Create a new directory template', 'directorist-gutenberg')}</h2>
                <p>{__('Choose your directory type - we\'ll generate the base templates for you.', 'directorist-gutenberg')}</p>

                <div className="directorist-gutenberg-choose-directory-type-options">
                    <h3>{__('Select Directory', 'directorist-gutenberg')}</h3>
                    <SelectControl
                        value={directoryType}
    ...
    const renderLoadingState = () => (
        <div className="directorist-gutenberg-create-template-modal-content-loading">
            ...
            <h2>{__('Please wait, your template is creating', 'directorist-gutenberg')}</h2>
            <span
                className="directorist-gutenberg-loading-progress"
                style={{ '--directorist-gutenberg-loading-progress': `${loadingProgress}%` }}
            />
        </div>
    );
    ...
    const renderFormContent = () => (
        <>
            ...
            <div className="directorist-gutenberg-create-template-modal-content-actions">
                ...
                <span>
                    {__('Each template (Grid, List, and Archive) can be edited in Gutenberg and enhanced using AI-assisted design conversation.', 'directorist-gutenberg')}
                </span>
            </div>
        </>
    );
    ...
    return (
        <StyledModalContents>
            ...
            <div className="directorist-gutenberg-create-template-modal-content">
                {isCreatingTemplate ? renderLoadingState() : renderFormContent()}
            </div>
        </StyledModalContents>
    );
}